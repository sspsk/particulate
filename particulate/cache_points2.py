...
def cache_points2(
    ...
    #face_subsequence = np.where(face_to_bone==0)[0].tolist()
    #submesh = mesh.submesh([face_subsequence],append=False)[0]
    ##submesh.export("shoecabinet_f7_main.obj")
    #debug_points, _, _, _, _, _ = sharp_sample_pointcloud(submesh, num_points_sharp_edges)
    #np.savez("debug_sharp_output.npz",points=debug_points)
    #exit()
